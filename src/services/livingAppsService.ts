...
import type { FeierDerZahl1, CreateFeierDerZahl1 } from '@/types/app';
...
export class LivingAppsService {
  // --- FEIER_DER_ZAHL_1 ---
  static async getFeierDerZahl1(): Promise<FeierDerZahl1[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FEIER_DER_ZAHL_1}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as FeierDerZahl1[];
    return enrichLookupFields(records, 'feier_der_zahl_1');
  }
  static async getFeierDerZahl1Entry(id: string): Promise<FeierDerZahl1 | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FEIER_DER_ZAHL_1}/records/${id}`);
    const record = { record_id: data.id, ...data } as FeierDerZahl1;
    return enrichLookupFields([record], 'feier_der_zahl_1')[0];
  }
  static async createFeierDerZahl1Entry(fields: CreateFeierDerZahl1) {
    return callApi('POST', `/apps/${APP_IDS.FEIER_DER_ZAHL_1}/records`, { fields: cleanFieldsForApi(fields as any, 'feier_der_zahl_1') });
  }
  static async updateFeierDerZahl1Entry(id: string, fields: Partial<CreateFeierDerZahl1>) {
    return callApi('PATCH', `/apps/${APP_IDS.FEIER_DER_ZAHL_1}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'feier_der_zahl_1') });
  }
  static async deleteFeierDerZahl1Entry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FEIER_DER_ZAHL_1}/records/${id}`);
  }
