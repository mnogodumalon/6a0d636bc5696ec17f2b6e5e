...
const FEIERDERZAHL1_FIELDS = [
  { key: 'vorname', label: 'Vorname', type: 'string/text' },
  { key: 'nachname', label: 'Nachname', type: 'string/text' },
  { key: 'datum_feier', label: 'Datum der Feier', type: 'date/datetimeminute' },
  { key: 'ort', label: 'Veranstaltungsort', type: 'string/text' },
  { key: 'teilnehmerzahl', label: 'Anzahl der Teilnehmer', type: 'number' },
  { key: 'anlass', label: 'Anlass der Feier', type: 'lookup/select', options: [{ key: 'erster_geburtstag', label: 'Erster Geburtstag' }, { key: 'erster_jahrestag', label: 'Erster Jahrestag' }, { key: 'erster_arbeitstag', label: 'Erster Arbeitstag' }, { key: 'erster_schultag', label: 'Erster Schultag' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'botschaft', label: 'Persönliche Botschaft', type: 'string/textarea' },
  { key: 'foto', label: 'Foto hochladen', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'feier_der_zahl_1', label: 'Feier der Zahl 1', pascal: 'FeierDerZahl1' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('feier_der_zahl_1');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'feier_der_zahl_1': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'feier_der_zahl_1': return (data as any).feierDerZahl1 as FeierDerZahl1[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'feier_der_zahl_1': return FEIERDERZAHL1_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'feier_der_zahl_1': return {
        create: (fields: any) => LivingAppsService.createFeierDerZahl1Entry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFeierDerZahl1Entry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFeierDerZahl1Entry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'feier_der_zahl_1' || dialogState?.entity === 'feier_der_zahl_1') && (
        <FeierDerZahl1Dialog
          open={createEntity === 'feier_der_zahl_1' || dialogState?.entity === 'feier_der_zahl_1'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'feier_der_zahl_1' ? handleUpdate : (fields: any) => handleCreate('feier_der_zahl_1', fields)}
          defaultValues={dialogState?.entity === 'feier_der_zahl_1' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['FeierDerZahl1']}
          enablePhotoLocation={AI_PHOTO_LOCATION['FeierDerZahl1']}
        />
      )}
      {viewState?.entity === 'feier_der_zahl_1' && (
        <FeierDerZahl1ViewDialog
          open={viewState?.entity === 'feier_der_zahl_1'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'feier_der_zahl_1', record: r }); }}
        />
      )}
